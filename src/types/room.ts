type RoomStore = {
  ...
  changeCard: ({
    socketId,
    card
  }: { socketId: string; card: number }) => void;
  startRoomVoting: (roomId: string) => void;
  setRoomsCards: ({
    roomId,
    newCards
  }: { roomId: string; newCards: RoomCards }) => void;
  ...
  createRoom: ({
    ...
    username,
    socketId
  }: { roomId: string; username: string; socketId: string }) => void;
  shouldShowCards: ({ roomId }: { roomId: string }) => boolean;
  setRoom: ({
    roomId,
    newRoomState
  }: { roomId: string; newRoomState: Room }) => void;
